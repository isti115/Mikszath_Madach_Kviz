    ...
    public partial class MainForm : Form
    {
        ...
        private void hintButton_Click(object sender, EventArgs e)
        {
            ...
            while (correct && i < (answers.GetLength(HORIZONTAL) * answers.GetLength(VERTICAL)))
            {
                ...
                {
                    int curr_x = (xStart * xAvgSize) + xSum(1 + xTry) + (xMargin / 2);
                ...
                {
                    int curr_y = (yStart + 1 + yTry) * yAvgSize + (yMargin / 2);
            ...
            for (int y = 0; y < answers.GetLength(VERTICAL); y++)
            {
                for (int x = 0; x < answers.GetLength(HORIZONTAL); x++)
                {
                    ...
                    currLabel.Location = new Point(
                        ...
                        rand.Next(((yStart + yCount) * yAvgSize) + (yMargin / 2), this.Height - (yAvgSize - (yMargin / 2)))
                        );
            ...
            for (int y = 0; y < answers.GetLength(VERTICAL); y++)
            {
                for (int x = 0; x < answers.GetLength(HORIZONTAL); x++)
                {
                    ...
                    if (xPos != -1 && yPos != -1)
                    {
                        if (currLabel.Text == answers[xPos, yPos])
                        {
                            currLabel.BackColor = Color.FromArgb(70, Color.Green);
                        }

                        else
                        {
                            currLabel.BackColor = Color.FromArgb(70, Color.Red);
                        }
                    }

                    else
                    {
                        currLabel.BackColor = Color.FromArgb(x * 25 + 50, Color.Blue);
                    }
                }
            }
